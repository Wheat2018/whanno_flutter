...
///
/// 提示：使用索引器访问和修改颗粒，可以完美避免注意事项二。
abstract class Dispatcher<Source, Granule> extends CacheGetter<List<Viewer<Granule>>> {
  Viewer<Source> get source;
...
abstract class TokenDispatcher<Source, Granule> extends Dispatcher<Source, Granule> implements Tokenize {
  @override
  List<TokenViewer<Granule>>? get() => super.get() as dynamic; // 强制绕过静态类型检查。

  @override
  List<TokenViewer<Granule>>? performGet();
...
class StringDispatcher extends TokenDispatcher<String, String> {
  ...
  String? rebuildSource() {
    ...
    return str.replaceAllMapped(pattern, (m) => matches[m.start].on(notNull: (i) => strImage[i]) ?? m.group(0) ?? "");
  }
}
...
class ExpandDispatcher<Source, Granule> extends TokenDispatcher<List<TokenDispatcher<Source, Granule>>, Granule> {
  ...
  final Viewer<List<TokenDispatcher<Source, Granule>>> source;
  ExpandDispatcher({required this.source});

  ExpandDispatcher.fromList(List<TokenDispatcher<Source, Granule>> list) : source = ValueViewer(list);

  @override
  List<TokenViewer<Granule>>? performGet() => source.get()?.map((e) => e.get()).skipNull().expand((e) => e).toList();

  @override
  List<TokenDispatcher<Source, Granule>>? rebuildSource() => source.get()?..forEach((e) => e.flash());
}
